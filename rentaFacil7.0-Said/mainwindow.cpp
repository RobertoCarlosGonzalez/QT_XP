    ...
    ui(new Ui::MainWindow)
{
    ...
    //---------------------------------------------------------------------------------------------
    id_client="1";
    ui->cb_deposito_pagar->addItem("2000");
        ...
        informacion.exec();
            ...
            qDebug()<<contra;
                ...
                if(ui->cliente->isChecked()){
                     consulta.append("SELECT  idcliente, nombreUsuario,contrasena FROM propietario where nombreUsuario='"+user1+"' ");
                     query.prepare(consulta);
                     query.exec();
                     query.next();
                     user=query.value(1).toString();
...
void MainWindow::on_crearSesion_clicked()
{
     ...
     }else if(!ui->propietario_2->isChecked()){
...
void MainWindow::on_Btn_reservar_seleccion_clicked()
{
    ...
    deposito = ui->cb_deposito_pagar->currentText();
...
//    insert.append("insert into reservacion (idCliente,idCasa,f_llegada,f_salida,deposito) values('"+ id_client +"','"+ idCasa +"','"+ f_llegada +"','"+ f_salida +"','"+ deposito +"')");
    query.prepare("insert into reservacion(idCliente,idCasa,f_llegada,f_salida,deposito) values('"+ id_client +"','"+ res +"','"+ f_llegada +"','"+ f_salida +"','"+ deposito +"')");
    query.exec();
    if(!query.next()){
        qDebug()<<"BIEN HECHO";
    }else{
        qDebug()<<"FALLASTE CRACK!"<<id_client<<" "<<res<<" "<<deposito<<" "<<f_llegada<<" "<<f_salida;
    }
