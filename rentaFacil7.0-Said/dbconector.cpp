...
void dbconector::registro_cliente(QString nombre, QString apellidoP, QString apellidoM, QString domicilio, QString telefono, QString nombreUsuario,QString contrasena){
    //query de insertado
    QSqlQuery insertar;
    insertar.exec("insert into cliente(nombre,ap,am,telefono,direccion,nombreUsuario,contrasena)values('"+ nombre +"','"+ apellidoP +"','"+ apellidoM +"','"+ telefono +"','"+ domicilio +"','"+ nombreUsuario +"','"+ contrasena +"')");
    if(!insertar.next())
        qDebug()<<"Registro cliente exitoso";
    else {
        qDebug()<<"Registro cliente fallido en query";
    }
