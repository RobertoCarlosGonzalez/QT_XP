            ...
            qDebug()<<contra;
                     ...
                     ui->paginas->setCurrentIndex(5);
                         ...
                         informacion.exec();
                              ...
                              consulta.append("SELECT  nombreUsuario,contrasena FROM propietario where nombreUsuario='"+user1+"' ");
                              query.prepare(consulta);
                              query.exec();
                              query.next();
...
void MainWindow::on_crearSesion_clicked()
{
     ...
     msgBox.setButtonText(QMessageBox::No,"No");
            ...
            if(ui->nombre->text()==""||ui->apellidoP->text()==""||ui->apellidoM->text()==""||ui->domicilio->text()==""||ui->telefono->text()==""||ui->correo->text()==""||ui->cuentabancaria->text()==""||ui->nombreUsuario_2->text()==""||ui->contrasena_1->text()==""||ui->propietario_2->isChecked()==false){
                informacion.setWindowTitle("Informacion");
                informacion.setText ("Campos incompletos,verifique");
                informacion.setStandardButtons( QMessageBox::Ok) ;
                informacion.setDefaultButton (QMessageBox ::Ok ) ;
...
void MainWindow::on_tablaCatalogo_doubleClicked(const QModelIndex &index)
{
    ...
    ui->lbDescripcion03->setText(q.value(4).toString());
    ui->lbTarifa03->setText(q.value(5).toString());
    ui->lbMascotas03->setText(q.value(7).toString());
    ui->lbColonia03->setText(q.value(8).toString());
   /*
    QString idcasa=q.value(0).toString();
    qDebug()<<idcasa<<"id casa";
    mostrarActividades(idcasa);
   */
    //ui->lbActividades03->setText(q.value(6).toString());
...
void MainWindow::on_finalizarRegistro_clicked()
{
    ...
    QMessageBox msgBox(QMessageBox::Question,"Confimacion","¿Has termina de agregar imagenes y actividades a tu casa?",QMessageBox::Yes|QMessageBox::No);
    msgBox.setButtonText(QMessageBox::Yes,"Sí");
    msgBox.setButtonText(QMessageBox::No,"No");
    if(msgBox.exec()==QMessageBox::Yes){
    ui->paginas->setCurrentIndex(5);
